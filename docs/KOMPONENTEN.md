...
## 12. Nachträge aus der Umsetzung (Stand 16.09.2026)

Diese Abweichungen wurden beim Bau gegen Freqtrade 2026.8, ccxt 4.5.78 und vLLM 0.29.0 festgestellt, geprüft und übernommen. Sie gelten ab jetzt als Teil des Vertrags.

| Abschnitt | Nachtrag |
|---|---|
| 2 | Freqtrade wird per `pip install freqtrade==2026.8` nach `/opt/freqtrade/venv` installiert (kein Git-Checkout); `install.sh` legt `/opt/freqtrade/.venv` als Symlink an. Der Dry-Run-Bot loggt nach `user_data/logs/freqtrade-dryrun.log`, der Live-Bot nach `freqtrade.log`. Zusätzliche Datei `/etc/freqtrade/secrets-guard.env` (root:freqtrade 0640), die nur `btctrader-guard.service` lädt |
| 3 | Neue Variablen: `GUARD_TRADE_API_KEY` / `GUARD_TRADE_API_SECRET` (Trade-Key nur für den optionalen `cancelOrdersAfter`-Dead-Man's-Switch, nie in `btctrader.env`), `GUARD_TRADE_ENV_FILE` (Default `/etc/freqtrade/secrets-guard.env`), `BTCTRADER_ENV_FILE` (Bootstrap: Pfad der Env-Datei selbst). `GUARD_COD_SECONDS` wird auf 10 bis 300 begrenzt |
| 4 | `protections` darf in Freqtrade 2026.8 nicht in `config.json` stehen; die drei Protections sind eine `protections`-Property in `BtcTrend`. Backtests brauchen `--enable-protections` (setzt `scripts/backtest.sh`). Bitvavo erlaubt in Freqtrade nur `order_time_in_force: GTC`; Maker-Ausführung wird über `price_side: "same"` plus Orderbuch angestrebt, nicht garantiert |
| 5 | Zeitstempel werden einheitlich mit sechs Nachkommastellen geschrieben (`2026-09-15T13:07:02.000000Z`), damit die String-Sortierung chronologisch ist. Ein Konto (`LEDGER_ACCOUNT_ID`) enthält nur Fills einer Art (`source`, `dry_run`); beim Wechsel auf `LEDGER_SOURCE=exchange` muss die Konto-Kennung wechseln (z. B. `bitvavo-live`), sonst bricht `sync` mit Fehler ab. Dry-Run-Fills heißen `ft-<order_id>` (ein Fill pro gefüllter Order). Ein Verkauf ohne passende Lots (z. B. eingezahlte BTC) wird als `fifo_error` in `sync_state` vermerkt, `daily` läuft weiter; die Tabelle `transfers` wird noch nicht in die FIFO einbezogen |
| 6 | Der `guided_json`-Fallback steht auf oberster Ebene des Request-Bodys. Das an vLLM gesendete Schema enthält keine `minimum`/`maximum`/`maxLength`; die Grenzen prüft Pydantic. Zusätzlich gesendet: `chat_template_kwargs.enable_thinking=false`. `decisions.jsonl` enthält zusätzlich `used_guided_json_fallback`. Indikatoren nutzen nur geschlossene Kerzen; die offene Kerze steht separat in `open_candle_1d` |
| 7 | Das Veto gegen Signal-Exits vor `min_hold_candles` sitzt in `confirm_trade_exit` (Freqtrade ruft `custom_exit` bei gesetztem Exit-Signal nicht auf). Kill-Switch-Sperre und Advisor-Gate greifen nur in `live`/`dry_run` und decken auch Rebalance-Käufe in `adjust_trade_position` ab. Rebalance-Verkäufe werden in Freqtrades Konvention zurückgegeben (negativer Stake relativ zu `trade.stake_amount`, Umrechnung in `btctrend_lib.partial_exit_stake`). Hilfsfunktionen liegen in `user_data/strategies/btctrend_lib.py` |
| 8 | Unter `killswitch.lock` sind Tagesverlust- und Drawdown-Trigger ausgesetzt; die Sperre setzt pro Lauf einmal `stopentry` ohne Alarm. Bleiben Positionen offen, wird `forceexit` (Market) bei jedem Lauf wiederholt und alarmiert. Equity wird bei fehlender Bewertung durch Freqtrade mit dem öffentlichen Ticker nachbewertet; ohne Ticker wird der Lauf ohne Limitprüfung übersprungen. `reset --confirm` setzt auch `peak_equity` zurück. Zusätzliche Events: `day_start`, `killswitch_reset`, `killswitch_open_trades`, `ft_recovered`, `action_failed`. Vor Ein- und Auszahlungen den Bot stoppen oder danach den Guard zurücksetzen, weil Equity = `balance.total` |
| 10 | Zusätzliche Fragmente `/partials/tiles`, `/partials/fills`, `/partials/events` für htmx. Das Dashboard öffnet `ledger.sqlite` read-only (`mode=ro`) und in einer Lesetransaktion; eine fehlende DB wird als leer angezeigt |
| 11 | Stand der Tests: 426 im Projekt-venv (common 100, ledger 55, advisor 66, guard 55, dashboard 60, deploy 32, strategy 35, Integration 23) plus 61 Strategie-Tests unter Freqtrade. Backtest-Bericht in `docs/BACKTEST.md` |
