...
does ``regime == "risk_off"`` block new exposure: the initial entry
(``confirm_trade_entry`` and ``custom_stake_amount``) and rebalance buys from
``adjust_trade_position``, which Freqtrade never routes through
``confirm_trade_entry``. The gate plugs into ``BtcTrend.entry_block_reason``.
...
with one warning per hour. Exits, including partial rebalance exits, are never
blocked by the advisor. The gate is inactive in backtesting and hyperopt
because a single current decision file has no meaning for historical candles.
...
class BtcAdvisorGated(BtcTrend):
    ...
    def entry_block_reason(self) -> str | None:
        reason = super().entry_block_reason()
        if reason is not None:
            return reason
        if getattr(self, "_gate_block", False):
            return f"blocked by advisor gate ({self._gate_reason}, decision {self._gate_decision_id})"
        return None
